...
var EventEmitter = require('events').EventEmitter,
    ...
    Warlock = require('node-redis-warlock'), // 分布式锁
    _ = require('lodash'),
    redis = require('./redis');
...
Queue.prototype.setupTimers = function() {
    if (this.warlock === undefined) {
        ...
        this.warlock = new Warlock(this.lockClient); // 分布式锁
    }
            ...
            if (typeof unlock === 'function') {
                ...
                client.zrangebyscore(client.getKey('jobs:active'), 100000, Date.now(), "LIMIT", 0, limit, function(err, ids) {
                    ...
                    ids.forEach(function(id) {
                        ...
                        Job.get(id, function(err, job) {
                            if (err) return doUnlock();
                            //var worker = self.workers.filter( function(w){ return w.type === job.type } ).pop();
                            if (self.workers.length > 0) {
                                self.workers[0].failed(job, {
                                    error: true,
 ...
 * @param {Number|Function} n // 并行数
 * @param {Function} fn
 * @api public
 */
Queue.prototype.process = function(type, n, fn) {
    console.log('Queue process start. Type: ' + type);
    var self = this;
    ...
    while (n--) {
        ...
        worker.id = [self.id, type, self.workers.length + 1].join(':'); //`queueId`:`type`:`index`
        ...
        worker.on('job complete', function(job) {
            ...
            if (self.client) {
                self.client.incrby(self.client.getKey('stats:work-time'), job.duration); // 运行时间
            }
        });
