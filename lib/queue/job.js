...
exports.disableSearch = true; // Job.disableSearch = true;


exports.jobEvents = true; // Job.jobEvents = true;
...
function getSearch() {
    ...
    return search = reds.createSearch(redis.client().getKey('search')); // q:search
}
...
function get(fn, order) {
    return function(err, ids) {
        ...
        var pending = ids.length,
            jobs = {};
        if (!pending) return fn(null, ids); // !0 true;
        ...
        ids.forEach(function(id) {
            exports.get(id, function(err, job) {
                ...
                --pending || fn(null, 'desc' == order ? map(jobs, ids).reverse() : map(jobs, ids)); // job ids []
...
exports.rangeByState = function(state, from, to, order, fn) {
    redis.client().zrange(redis.client().getKey('jobs:' + state), from, to, get(fn, order)); // SET zrange q:job:`stat`
};
 ...
 * 返回任务的所有信息 HGETALL q:job:901125
 *
...
exports.get = function(id, fn) {
    ...
    client.hgetall(client.getKey('job:' + job.id), function(err, hash) {
        ...
        job.type = hash.type;
        job._ttl = hash.ttl; //
        job._delay = hash.delay; // 延时
        job.priority(Number(hash.priority)); // 优先级
        job._progress = hash.progress; // 进度
        job._attempts = hash.attempts; // 已经重试的次数
        job._max_attempts = hash.max_attempts; // 最大重试次数
        job._state = hash.state; // 任务状态
        ...
        try {
            ...
            if (hash.backoff) {
                var source = 'job._backoff = ' + hash.backoff + ";"; // backoff函数也存在redis中
...
exports.removeBadJob = function(id) {
    var client = redis.client();
    client.multi()
        .del(client.getKey('job:' + id + ':log')) // q:job:901125:log
        .del(client.getKey('job:' + id)) // q:job:901125
        .zrem(client.getKey('jobs:inactive'), id) // q:job:inactive:901125
        .zrem(client.getKey('jobs:active'), id) // q:job:active:901125
        .zrem(client.getKey('jobs:complete'), id)  // q:job:complete:901125
        .zrem(client.getKey('jobs:failed'), id) // q:job:failed:901125
        .zrem(client.getKey('jobs:delayed'), id) // q:job:delayed:901125
        .zrem(client.getKey('jobs'), id) // q:jobs:901125
        .exec();
...
Job.prototype.log = function(str) {
    ...
    this.client.rpush(this.client.getKey('job:' + this.id + ':log'), str); // RPUSH q:job:901125:log 'log'
...
Job.prototype.set = function(key, val, fn) {
    this.client.hset(this.client.getKey('job:' + this.id), key, val, fn || noop); // q:job:901125
    return this;
};

/**
 * Get job `key`
 * Job的内容存在一个map中，key为map的key
 * @param {String} key
 * @param {Function} fn
 * @return {Job} for chaining
 * @api public
 ...
 * 设置或获取Job的进度，适合长时间处理的任务
 *
 * @param {Number} complete // 已完成的
 * @param {Number} total // 所有的
 * @param {Object} data // 一些数据，可选
 * @return {Job} for chaining
 * @api public
 */
...
Job.prototype.priority = function(level) {
    if (0 == arguments.length) return this._priority; // 没有参数时获取优先级
 ...
 *
 * @param {Function} fn (err, max(max_attempts, attempts), attempts, max_attempts)
 * @return {Job} for chaining
 * @api public
 */
...
Job.prototype.attempt = function(fn) {
    var self = this,
        client = this.client,
        id = this.id,
        key = client.getKey('job:' + id); // q:job:`id`

    //TODO this now can be removed, since max_attempts is set in the constructor
    // 将哈希表 key 中的域 field 的值设置为 value ，当且仅当域 field 不存在。
    // 若域 field 已经存在，该操作无效。
    // 如果 key 不存在，一个新哈希表被创建并执行 HSETNX 命令。
    client.hsetnx(key, 'max_attempts', 1, function() { // 如果没有设置一个1
        client.hget(key, 'max_attempts', function(err, max) { //
            client.hincrby(key, 'attempts', 1, function(err, attempts) { // 自增一个
...
Job.prototype.remove = function(fn) {
    ...
    client.multi()
        .zrem(client.getKey('jobs:' + this.state()), this.id) // q:jobs:`state`:901125
        .zrem(client.getKey('jobs:' + this.type + ':' + this.state()), this.id) // q:jobs:`type`:`state`:id
        .zrem(client.getKey('jobs'), this.id) // q:jobs:id
        .del(client.getKey('job:' + this.id + ':log')) // q:job:901125:log
        .del(client.getKey('job:' + this.id)) // q:job:901125
        .exec(function(err) {
            // events.remove(this);
            if (!exports.disableSearch) {
...
Job.prototype.state = function(state, fn) {
    console.log('Job state. Type: ' + this.type + ', Id: ' + this.id);
    if (0 == arguments.length) return this._state; // 不传参数，返回此任务的状态
    ...
    if (oldState && oldState != '' && oldState != state) {
        multi
            .zrem(client.getKey('jobs:' + oldState), this.id) // q:jobs:`oldState`:id
            .zrem(client.getKey('jobs:' + this.type + ':' + oldState), this.id); // q:jobs:`type`:`oldState`:id
    }

    multi
        .hset(client.getKey('job:' + this.id), 'state', state)
        .zadd(client.getKey('jobs:' + state), this._priority, this.id) // q:jobs:`state` 优先级 jobID
        .zadd(client.getKey('jobs:' + this.type + ':' + state), this._priority, this.id);

    // use promote_at as score when job moves to delayed. promote_at = now + delayed
    ('delayed' === state) ? multi.zadd(client.getKey('jobs:' + state), parseInt(this.promote_at), this.id): noop();
    ('active' === state && this._ttl > 0) ? multi.zadd(client.getKey('jobs:' + state), Date.now() + parseInt(this._ttl), this.id): noop(); // q:jobs:`state`
    ('inactive' === state) ? multi.lpush(client.getKey(this.type + ':jobs'), 1): noop(); // lpush q:`type`:jobs 1
    ...
    multi.exec(function(err, replies) {
        ...
        console.log('Job save over. Type: ' + this.type + ', Id: ' + this.id);
        return fn(err);
    }.bind(this));
    return this;
};
...
Job.prototype.save = function(fn) {
    console.log('Job save start. Type is ' + this.type);

    var client = this.client,
        fn = fn || noop,
        max = this._max_attempts, // 最大尝试次数
        self = this;

    // update
    if (this.id) return this.update(fn); // 如果存在则更新
    ...
    client.incr(client.getKey('ids'), function(err, id) {
        ...
        if (err) console.log(err);
        console.log('Job save. Type: ' + self.type + ', Id: ' + id);
        // add the job for event mapping
        var key = client.getKey('job:' + id); // q:job:901125
        self.id = id;
        self.subscribe(function() {
            self._state = self._state || (this._delay ? 'delayed' : 'inactive');
            if (max) client.hset(key, 'max_attempts', max);
            client.sadd(client.getKey('job:types'), self.type); // SET s:job:types 将新的消息类型放到redis中
            self.set('type', self.type); // hset q:job:901125 type 'type'
            var now = Date.now();
            self.created_at = now;
            self.set('created_at', self.created_at); // hset q:job:901125 craete_at 'create_at'
            self.promote_at = now + (self._delay || 0); // 当前时间+延迟时间
...
Job.prototype.update = function(fn) {
    console.log('Job update. Type: ' + this.type + ', Id: ' + this.id);
    var json;

    // serialize json data
    try {
        json = JSON.stringify(this.data); // 数据序列化
    } catch (err) {
        fn(err);
        return this;
    }
