 ...
 *
 * @param {Function} fn (job, done)处理函数
 * @return {Worker} for chaining
 * @api private
 */
Worker.prototype.start = function(fn) {
    console.log('Worker start. Type: ' + this.type);

    this._fn = fn;
    var self = this;
    self.idle(); // 清空初始化
    if (!self.running) return;
    this.queue.removeAllListeners('job ttl exceeded'); // 移除监听器job ttl exceeded
    /*
 ...
 *
 * @param {Job} job
 * @param {Object} theErr err
 * @param {Function} fn (err, done)
 * @return {Worker} for chaining
 * @api private
 */
...
Worker.prototype.failed = function(job, theErr, fn) {
    var self = this;
    job.error(theErr).failed(function() {
        job.attempt(function(error, remaining, attempts /*, max*/ ) {
            if (error) return self.error(error, job);
            if (remaining) {
                job.reattempt(attempts, function(err) {
                    if (err) return self.error(err, job);
                    self.emitJobEvent('failed attempt', job, theErr.message || theErr.toString(), attempts);
                });
            } else {
...
Worker.prototype.process = function(job, fn) {
    console.log('Worker process. Type: ' + this.type + ', Id: ' + job.id);
    var self = this,
        start = new Date();
    ...
    var createDoneCallback = function(jobId) {
        ...
        return function(err, result) {
            console.log('Worker process. Job Done. Type: ' + self.type + ', Id: ' + jobId);
 ...
 *
 * @param {Function} fn (err, job)
 * @api private
 */
Worker.prototype.getJob = function(fn) {
    console.log('Worker getJob. Type: ' + this.type + ', running: ' + this.running);
    var self = this;
    if (!self.running) {
    ...
    // 每个通道分配一个连接
    var client = clients[self.type] || (clients[self.type] = redis.createClient()); // 每个管道一个客户端
    // BLPOP indicates we have a new inactive job to process
    client.blpop(client.getKey(self.type + ':jobs'), 0, function(err) { // q:`type`:jobs
        if (err) console.log(err);
        if (err || !self.running) {
            self.client.lpush(self.client.getKey(self.type + ':jobs'), 1); // lpush q:`type`:jobs 1
            return fn(err); // SAE: Added to avoid crashing redis on zpop
        }

        console.log('Worker getJob. poped Type: ' + self.type);
        // Set job to a temp value so shutdown() knows to wait
        self.job = true;
        // 弹出来一个待处理的任务
        self.zpop(self.client.getKey('jobs:' + self.type + ':inactive'), function(err, id) { // q:jobs:`type`:inactive 待处理的job
            if (err || !id) {
                self.idle();
                return fn(err /*|| "No job to pop!"*/ );
            }
            console.log('Worker getJob. Type: ' + self.type + ', Id: ' + id);
            Job.get(id, fn);
        });
    });
};
...
Worker.prototype.emitJobEvent = function(event, job, arg1, arg2) {
    ...
    this.emit('job ' + event, job); // job complete
};
